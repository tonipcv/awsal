  ...
  for (const progress of oldProgress) {
    ...
    await prisma.protocolDayProgressV2.create({
      data: {
        ...
        status: progress.completed || progress.isCompleted ? 'COMPLETED' : 'PENDING',
...
async function updateAdherenceRates() {
  ...
  for (const prescription of prescriptions) {
    ...
    const completedDays = prescription.progressV2.filter(p => p.status === 'COMPLETED').length;
    const missedDays = prescription.progressV2.filter(p => p.status === 'MISSED').length;
