    ...
    if (!settings) {
      settings = await prisma.aIAssistantSettings.create({
        data: {
          ...
          welcomeMessage: 'Olá! Sou o assistente de IA do Dr. {doctorName}. Como posso ajudá-lo hoje?',
          fallbackMessage: 'Desculpe, não consegui encontrar uma resposta adequada. Sua pergunta foi encaminhada para o médico.',
          maxDailyMessages: 50
        }
      });
    }
  ...
  try {
    ...
    const settings = await prisma.aIAssistantSettings.upsert({
      ...
      create: {
        ...
        welcomeMessage: welcomeMessage ?? 'Olá! Sou o assistente de IA do Dr. {doctorName}. Como posso ajudá-lo hoje?',
        fallbackMessage: fallbackMessage ?? 'Desculpe, não consegui encontrar uma resposta adequada. Sua pergunta foi encaminhada para o médico.',
